...
2. Calls:
   ...
   - Uses a 2-day UTC window (`start_date = yesterday`, `end_date = today`) to avoid day-boundary misses.
   - Header: `Authorization: Bearer $OURA_ACCESS_TOKEN`
3. Resolves a best daily row from `.data[]`:
   - Prefer a row for `end_date` with non-zero steps.
   - Fallback to the most recent row with numeric steps.
   - Steps are read from `.steps` first, then `.contributors.steps`.
